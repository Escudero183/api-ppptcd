...
public interface InstitutionDirectoryRepository extends JpaRepository<InstitutionDirectory, Integer> {
    ...
    @Query("select t from InstitutionDirectory t Where t.status = true and (:idEducationalInstitution = -1 or (:idInstitution != -1 and (t.idInstitution = :idInstitution))) and (lower(t.firstName) like :query or lower(t.lastNameOne) like :query or lower(t.lastNameTwo) like :query)")
